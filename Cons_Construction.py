...
OUTPUT_CSV_FILE = '经纬度(Iridium)new.csv'     # 输出的CSV文件名

# Scenario Time
SCENARIO_START_TIME = '12 Oct 2025 00:00:00.00' # 仿真开始时间
SCENARIO_STOP_TIME = '12 Oct 2025 01:00:00.00'   # 仿真结束时间
...
TIME_STEP_SEC = 0.1    # 经纬度数据采样的时间步长（秒）。例如，3600代表每小时采样一次。
...
def export_satellite_trajectories(scenario, root):
    ...
    for i in range(satellites.Count):
        ...
        for time, lat, lon in zip(time_values, lat_values, lon_values):
            ...
            frame_idx = int(round(time / TIME_STEP_SEC))
            trajectory_data.append([sat_name, lat, lon, frame_idx])
